...
export const taskService = {
  ...
  bulkMarkDone(filters: { status?: string; category_id?: string; search?: string }) {
    return api.patch('/tasks/bulk/done', filters)
  },

  getAllAdmin(filters: TaskFilters & { user_id?: number }) {
    return api.get<ApiResponse<{ tasks: Task[]; pagination: Pagination }>>('/tasks/all', {
      params: filters,
    })
  },
}