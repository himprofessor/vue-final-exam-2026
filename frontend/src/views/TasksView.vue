    ...
    <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div class="relative w-full">
        <BaseInput
          ...
          @update:model-value="taskStore.searchTasksDebounced"
        />

        <button
          v-if="taskStore.filters.search"
          type="button"
          class="absolute right-3 bottom-2.5 text-xs font-semibold text-gray-400 hover:text-gray-600 transition-colors duration-150 z-20"
          @click="taskStore.searchTasksDebounced('')"
        >
          ✕
        </button>
      </div>
